...
function getSumBetweenNumbers(n1, n2) {
  const el = n2 - n1 + 1;
  const arr = Array.from({ length: el }, (_, i) => n1 + i);
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const topLess = rect1New.top <= rect2New.top;
  const topWidthMore = rect1New.top + rect1New.width >= rect2New.top;
  const leftLess = rect1New.left <= rect2New.left;
  const leftHeightMore = rect1New.left + rect1New.height >= rect2New.left;

  return topLess && topWidthMore && leftLess && leftHeightMore;
}
...
function findFirstSingleChar(str) {
  return (
    str
      .split('')
      .filter(
        (c, i) => str.indexOf(c) === i && str.indexOf(c, i + 1) === -1,
      )[0] || null
  );
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${before}${smaller}, ${bigger}${after}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  const newArr = digits.map((item, i) => {
    const dig = parseInt(item, 10);
    let doubled = dig * 2;

    if (doubled > 9) doubled -= 9;

    return i % 2 !== 0 ? doubled : dig;
  });

  const res = newArr.reduce((acc, i) => acc + i, 0);
...
function getDigitalRoot(num) {
  ...
  const firstDigit = Number(String(res)[0]) || 0;
  const secondDigit = Number(String(res)[1]) || 0;
